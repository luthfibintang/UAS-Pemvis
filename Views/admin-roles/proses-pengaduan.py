...
pengaduan_data = adminbcknd.get_pengaduan_verified()

# Create header labels
for i, col in enumerate(columns):
    label = CTkLabel(table_frame, text=col, text_color="black", font=("PlusJakartaSans", 13, "bold"), width=145.5, height=50, fg_color="#D3D3D3")
    label.grid(row=0, column=i, sticky="nsw")

# Populate the table with data
for row_idx, row_data in enumerate(pengaduan_data, start=1):
    label = CTkLabel(table_frame, text=str(row_idx), text_color="black", font=("PlusJakartaSans", 13), width=145.5, height=50)
    label.grid(row=row_idx, column=0, sticky="nsw")
    
    for col_idx, item in enumerate([row_data['nik'],row_data['judul'], row_data['tgl_pengaduan']], start=1):
        label = CTkLabel(table_frame, text=item, text_color="black", font=("PlusJakartaSans", 13), width=145.5, height=50)
    ...
    detail_btn.pack(side=LEFT, padx=(5, 2), pady=10)
    
    if row_data['status'] == "Terverifikasi":
        proses_img = CTkImage(dark_image=Image.open("Assets/icon/settings.png"), size=(18, 18))
        proses_btn = CTkButton(button_frame, image=proses_img, text="", command=lambda id_pengaduan=row_data['id_pengaduan']: proses_pengaduan(id_pengaduan), width=30, height=30, fg_color="#FFA426", hover_color="#BE7E24")
        proses_btn.pack(side=LEFT, padx=(4, 2), pady=10)
    
    if row_data['status'] == "Sedang Diproses":
        tanggapiPengaduan_img = CTkImage(dark_image=Image.open("Assets/icon/reply.png"), size=(16, 16))
        tanggapiPengaduan_btn = CTkButton(button_frame, image=tanggapiPengaduan_img, text="", command=lambda id_pengaduan=row_data['id_pengaduan']: tanggapi_pengaduan(id_pengaduan), width=30, height=30, fg_color="#47C363", hover_color="#3BA052")
        tanggapiPengaduan_btn.pack(side=LEFT, padx=(4, 2), pady=10)
