...
selectedmenu.place(x=0, y=300.5)

response_img = CTkImage(dark_image=Image.open("Assets/icon/response1.png"), size=(24, 24))
response_lab = CTkLabel(selectedmenu, image=response_img, text="", cursor="hand2")
response_lab.place(x=32, y=305.75 - 291.5)
response_lab.bind("<Button-1>", lambda e: open_tanggapanPengaduan())

response_lbl = CTkLabel(selectedmenu, text="Tanggapan", text_color="black", font=("PlusJakartaSans", 12), cursor="hand2")
response_lbl.place(x=64, y=305.75 - 291.5)
...
pengaduan_data = adminbcknd.get_pengaduan_selesai()
...
for row_idx, row_data in enumerate(pengaduan_data, start=1):
    ...
    detail_btn.pack(side=LEFT, padx=(40, 2), pady=10)
