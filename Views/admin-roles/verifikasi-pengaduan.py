...
pengaduan_data = adminbcknd.get_pengaduan_unverified()
...
for row_idx, row_data in enumerate(pengaduan_data, start=1):
    ...
    verifikasi_img = CTkImage(dark_image=Image.open("Assets/icon/check3.png"), size=(18, 18))
    verifikasi_btn = CTkButton(button_frame, image=verifikasi_img, text="", command=lambda id_pengaduan=row_data['id_pengaduan']: verifikasi_pengaduan(id_pengaduan), width=30, height=30, fg_color="#47C363", hover_color="#3BA052")
    verifikasi_btn.pack(side=LEFT, padx=(4, 2), pady=10)
    
    tolakPengaduan_img = CTkImage(dark_image=Image.open("Assets/icon/declined.png"), size=(16, 16))
    tolakPengaduan_btn = CTkButton(button_frame, image=tolakPengaduan_img, text="", command=lambda id_pengaduan=row_data['id_pengaduan']: tolak_pengaduan(id_pengaduan), width=30, height=30, fg_color="#FC544B", hover_color="#CB4139")
    tolakPengaduan_btn.pack(side=LEFT, padx=(4, 2), pady=10)
